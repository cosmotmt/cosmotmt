...
export default function GlobalPlayer() {
  ...
  // Infinity判定または読み込み中判定
  const isDurationValid = duration > 0 && duration !== Infinity;
  const showLoading = !isDurationValid && !isPlaying;
              ...
              <div className="flex items-baseline justify-between gap-4">
                ...
                <div className="text-[9px] md:text-[10px] text-gray-500 tracking-tighter whitespace-nowrap">
                  {showLoading ? (
                    ...
                    <>{formatTime(displayTime)} / {isDurationValid ? formatTime(duration) : "--:--"}</>
                  )}
                </div>
              </div>
              
              <div className={`relative h-6 md:h-8 flex items-center group ${!showLoading ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}>
                <input
                  type="range"
                  min="0"
                  max={isDurationValid ? duration : 0}
                  step="any"
                  value={displayTime}
                  ...
                  disabled={showLoading || !isDurationValid}
                ...
                <div className="relative w-full h-1.5 bg-white/10 border border-white/5 overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${isDurationValid ? (displayTime / duration) * 100 : 0}%` }}
                  >
